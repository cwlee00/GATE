...
from utils.util import AverageMeter, extract_embs_multi_noCLS, print_eval_result_by_groups_and_k, evaluate_metric, find_kNN
...
def set_local_config(cfg, args):
    ...
    # Log
    cfg.wandb = False
    cfg.experiment_name = f'ContrastiveOrder+{cfg.lambda_nll}*nllLoss_{cfg.lambda_center}*centerLoss_centerMetric{cfg.metric}_ver_{cfg.version}_FeatSim{cfg.similarity_type}_epsilon{cfg.epsilon}_Temp{cfg.temp}_lrDecay{cfg.lr_decay_rate}_labelDiff{cfg.label_diff}'
    cfg.save_folder = f'../real_final_multi_noCLS/{cfg.dataset}_fold{cfg.fold}/Epochs{cfg.epochs}_PREFIX_{cfg.margin}_tau{cfg.tau}_{cfg.model}_{cfg.backbone}_lr{cfg.learning_rate}_{get_current_time()}'
...
def main():
    ...
    for epoch in range(cfg.epochs):
        ...
        print('==> validation...')
        metrics = validate_AGE(loader_dict, model, cfg)
        # === 여러 데이터셋의 결과를 파일 이름에 반영 ===
        metric_strs = []
        for dataset_name, vals in metrics.items():
            mae = vals['mae']
            best_k = vals['best_k']
            metric_strs.append(f"{dataset_name}_MAE{mae:.2f}_k{best_k}")

        metrics_summary = "_".join(metric_strs)
        save_name = f"ep_{epoch}_{metrics_summary}.pth"
        save_ckpt(cfg, model, save_name)
...
def train(epoch, train_loader, model, optimizer, criterion, cfg, prev_loss_record):
    ...
    for idx, (images, _, ranks, _, _) in enumerate(train_loader):
        if cfg.augmentation:
...
def validate_AGE(loader_dict, model, cfg):
    ...
    embs_train = extract_embs_multi_noCLS(model, loader_dict['train_for_val'])
    embs_train = embs_train.cuda()

    embs_test = extract_embs_multi_noCLS(model, loader_dict['val'])
    ...
    for dataset_name in cfg.dataset:
        ...
        metrics[f'{dataset_name}'] = {'mae': mae, 'cs': cs, 'acc': acc, 'best_k': best_k}
